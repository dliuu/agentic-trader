...
class UWClient:
    ...
    def __init__(self, api_token: str, rate_limiter: TokenBucketRateLimiter):
    ...
    async def get_flow_alerts(
        ...
        resp = await uw_get(
            self._client,
            f"{self.BASE_URL}{self.FLOW_ALERTS}",
    ...
    async def get_dark_pool_recent(self) -> list[DarkPoolPrint]:
        ...
        resp = await uw_get(
            self._client,
            f"{self.BASE_URL}{self.DARK_POOL_RECENT}",
            limiter=self._limiter,
        )
        resp.raise_for_status()
        data = resp.json()
    ...
    async def get_market_tide(self) -> MarketTide:
        ...
        resp = await uw_get(
            self._client,
            f"{self.BASE_URL}{self.MARKET_TIDE}",
            limiter=self._limiter,
        )
        resp.raise_for_status()
        data = resp.json()
